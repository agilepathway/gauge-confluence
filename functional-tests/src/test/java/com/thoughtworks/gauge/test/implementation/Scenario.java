...
public class Scenario {
    ...
    public void addContextToSpecification1(String scenarioName, String specName, Table steps) throws Exception {
        new SpecificationBuilder().withScenarioName(scenarioName).withSpecName(specName)
                .withScenarioDataStoreWriteStatement("specName", specName)
                .withScenarioDataStoreWriteStatement("Scenario name", scenarioName)
                .withScenarioDataStoreWriteStatement(scenarioName, steps.toString()).withSteps(steps)
    ...
    public void addContextToSpecification(String scenarioName, String specName, String subFolder, Table steps)
            throws Exception {
        new SpecificationBuilder().withScenarioName(scenarioName).withSpecName(specName).withSteps(steps)
                .withSubDirPath(subFolder).buildAndAddToProject(false);
    }
